...
ind = 0;
for i = 1 : node_count
    node_weights1 = node_weights(i, :);
    if any(node_weights1(1 : n1))
        node_feats(i, 1 : cr) = weighted_pooling(...
            RS(1 : n1, :), node_weights1(1 : n1), params.pooling_mode);
    end
    
    if any(node_weights1((n1 + 1) : n2))
        node_feats(i, (cr + 1) : end) = weighted_pooling(...
            PS((n1 + 1) : n2, :), ...
            node_weights1((n1 + 1) : n2), ...
            params.pooling_mode);
    end
    
    edge_weights1 = edge_weights(i, :);
    val1 = weighted_pooling(...
        ES', edge_weights1, params.pooling_mode);
    val1 = full(val1);
    col_idx1 = code_idx(val1 ~= 0);
    row_idx1 = ones(length(col_idx1), 1) * i;
    val1 = val1(val1 ~= 0);

    % Grows pre-allocated memory for the index and value array.
